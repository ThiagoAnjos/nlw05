...
function call(id) {
  ...
  socket.emit("admin_list_messages_by_user", params, messages =>{    
    divMessages = document.getElementById(`allMessages${connection.user_id}`);
    ...
    messages.forEach(message => {
      ...
      if(message.admin_id === null){
        ...
        createDiv.innerHTML = `Atendente: ${message.text}</span>`;
        createDiv.innerHTML += `<span class="admin_date">${dayjs(message.created_at).format("DD/MM/YYYY HH:mm:ss")}</span>`
      }
...
function sendMessage(id) {
  ...
  const createDiv = document.createElement("div");        
  createDiv.className = "admin_message_admin";
  createDiv.innerHTML = `Atendente: ${params.text}</span>`;
...
socket.on("admin_receive_message", data => {
  const connection = connectionsUsers.find(connection =>  socket_id = data.socket_id);
